...
export default function Header({ onPlayClick }) {
  ...
  // Đã vá lại đường dẫn API bị đứt đoạn của bạn
  useEffect(() => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 w-full px-4 md:px-6 py-1.5 flex items-center justify-between bg-indigo-950/30 backdrop-blur-md border-b border-white/10 shadow-lg">
      {/* ── LOGO BÊN TRÁI ── */}
      <Link
        to="/"
        className="flex items-center gap-2.5 transition-transform duration-200 hover:scale-105 active:scale-95"
      >
        <img
      ...
      <nav className="hidden lg:flex items-center gap-4">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <span className="text-sm filter drop-shadow-md">{item.icon}</span>
